    ...
    public class DockerCommandExecutor
    {
        ...
        public void RunContainer(DockerBase dockerBase, int timeoutInMilliseconds = 120000)
        {
            try
            {
                ...
                string volumeFlags = "";
                if (!string.IsNullOrWhiteSpace(dockerBase.DockerVolume))
                {
                    var splits = dockerBase.DockerVolume.Split(new[] { ';', '|' }, StringSplitOptions.RemoveEmptyEntries);
                    foreach (var v in splits)
                    {
                        ...
                        if (parts.Length == 2)
                        {
                            ...
                            var cont = parts[1].Trim();
                            if (host.Length > 0 && cont.Length > 0)
                            {
                                if (host.Contains(' ')) host = $"\"{host}\"";
                                volumeFlags += $"-v {host}:{cont} ";
                            }
                        }
                    }
                }
                ...
                string command = $@"docker run -d " +
                                 ...
                                 (string.IsNullOrWhiteSpace(dockerBase.DockerNetwork) ? "" : $"--network {dockerBase.DockerNetwork} ") +
                                 envVars +
                                 volumeFlags +
        ...
        public void CreateNetwork(string networkName, int timeoutInMilliseconds = 30000)
        {
            try
            {
                ...
                // Idempotent: check if network already exists
                var inspectResult = _commandExecutor.RunCommandAndCaptureOutput($"docker network inspect {networkName}");
                ...
                // Create new network; ignore 'already exists' race condition errors
                string createNetworkCommand = $"docker network create {networkName}";
                var createResult = _commandExecutor.RunCommandAndCaptureOutput(createNetworkCommand, null, null, timeoutInMilliseconds);
                if (createResult.ExitCode != 0)
                {
                    var combinedError = string.Join("\n", createResult.Errors ?? new List<string>());
                    if (!combinedError.Contains("already exists", StringComparison.OrdinalIgnoreCase))
                    {
                        throw new Exception(string.IsNullOrWhiteSpace(combinedError) ? $"Failed to create network {networkName}" : combinedError);
                    }
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Error creating network {networkName}. Details: {ex.Message}");
            }
        }
        ...
        public void RemoveNetwork(string networkName)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(networkName)) return;
                // Check existence first; if not found skip
                var inspectResult = _commandExecutor.RunCommandAndCaptureOutput($"docker network inspect {networkName}");
                if (inspectResult.ExitCode != 0) return; // Not existing

                string removeNetworkCommand = $"docker network rm {networkName}";
                var removeResult = _commandExecutor.RunCommandAndCaptureOutput(removeNetworkCommand);
                if (removeResult.ExitCode != 0)
                {
                    var combinedError = string.Join("\n", removeResult.Errors ?? new List<string>());
                    if (!combinedError.Contains("No such network", StringComparison.OrdinalIgnoreCase))
                    {
                        throw new Exception(string.IsNullOrWhiteSpace(combinedError) ? $"Failed to remove network {networkName}" : combinedError);
                    }
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Error removing network {networkName}. Details: {ex.Message}");
            }
        }
        #endregion
        ...
        public void CopyFolderToContainer(string sourceDirectory, string containerName, string destinationDirectoryInContainer, int timeoutInMilliseconds = 60000)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(destinationDirectoryInContainer))
                    throw new Exception("Destination directory in container is required");

                // Ensure destination directory exists inside container
                MakeDirectory(containerName, destinationDirectoryInContainer, 10000);

                // docker cp behaves differently for folder. Use trailing /. to copy contents, not folder itself.
                ...
                string cpCommand = $"docker cp \"{normalizedSource}{Path.DirectorySeparatorChar}.\" {containerName}:\"{destinationDirectoryInContainer}\"";

                var success = _commandExecutor.RunCommand(cpCommand, null, null, timeoutInMilliseconds);
                if (!success)
                {
                    throw new Exception("docker cp command failed");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Error while copying folder to container. Details: {ex.Message}");
            }
        }
    }
}
