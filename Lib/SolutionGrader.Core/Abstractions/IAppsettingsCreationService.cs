...
public interface IAppsettingsCreationService
{
    ...
    /// <returns>Tuple containing proxy port and server port</returns>
    (int ProxyPort, int ServerPort) GenerateAppsettings(DatabaseConfiguration? dbConfig, string? clientExePath, string? serverExePath, EnvironmentConfiguration? envConfig, string? protocol);
