...
public sealed class NetworkMonitorService : INetworkMonitorService
{
    ...
    /// <summary>
    /// Checks if a TCP packet is a health check or keepalive packet that should be filtered out.
    /// These packets are generated by Windows or Docker engine to check port status and
    /// should not be captured as part of the actual client-server communication.
    /// 
    /// Filtered packets include:
    /// - TCP keepalive packets (ACK-only with zero or minimal payload)
    /// - Window probe packets
    /// - Retransmission of ACK packets with no new data
    /// </summary>
    private static bool IsHealthCheckOrKeepalive(TcpPacket tcpPacket)
    {
        // Skip TCP keepalive packets - these are ACK-only with no payload or single byte payload
        // Used by Windows/Docker to check if the connection is still alive
        ...
        if (isAckOnly)
        {
            ...
            // Pure ACK with no payload - likely keepalive or window update
            if (payloadLength == 0)
            {
                // Don't filter out connection establishment ACKs (they're part of handshake)
                // But filter out standalone ACKs that come later (keepalives)
                // We allow ACKs through initially but the context will determine if they're relevant
                return false; // Let ACKs through - they may be part of the handshake
            }
            
            // TCP keepalive probes often have 1 byte of garbage data
            if (payloadLength == 1)
            {
        ...
        // RST packets without data are usually connection cleanup, not health checks
        // but we should still capture them for debugging purposes
        
        return false;
    }
    
    /// <summary>
    /// Checks if a payload contains health check patterns that should be filtered out.
    /// These include HTTP health check endpoints and Docker/Kubernetes probes.
    /// </summary>
    private static bool IsHealthCheckPayload(string? payload)
    {
        if (string.IsNullOrEmpty(payload))
            return false;
        
        // Common health check URL patterns
        var healthCheckPatterns = new[]
        {
            "/healthz",           // Kubernetes-style health check
            "/health",            // Generic health endpoint
            "/ping",              // Docker ping endpoint
            "/ready",             // Readiness probe
            "/live",              // Liveness probe
            "/_ping",             // Docker internal ping
            "/status",            // Status endpoint (might be a false positive, but rare in student code)
            "HEAD /",             // HTTP HEAD requests often used for health checks
        };
        ...
        // Docker Desktop health check pattern
        if (payload.StartsWith("GET / HTTP", StringComparison.OrdinalIgnoreCase) && 
            payload.Contains("Connection: close", StringComparison.OrdinalIgnoreCase) &&
            payload.Length < 200) // Health checks are usually short
        {
            return true;
        }
